...
import { type BreadcrumbItem, NoteCategory } from '@/types';
import type { NoteCategoriesProps } from '@/types/pages/notes';
...
export default function NoteCategories({ categories, filters }: NoteCategoriesProps) {
    ...
    const { search, setSearch, perPage, setPerPage, reset: resetDataView } = useDataViewSearch('note-categories.index', filters);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingCategory) {
            ...
            post(route('note-categories.store'), {
                onSuccess: () => {
                    setShowModal(false);
                    reset();
                },
    ...
    const confirmSave = () => {
        if (editingCategory) {
            put(route('note-categories.update', editingCategory.id), {
                onSuccess: () => {
    ...
    const columns: DataViewColumn<NoteCategory>[] = [
        {
            key: 'actions',
            label: 'Actions',
            align: 'center',
            render: (c) => <TableRowActions id={c.id} label={c.name} edit={{ onClick: () => openEditModal(c) }} deleteRoute="note-categories.destroy" />,
        },
        {
            key: 'name',
            label: 'Name',
    ...
    const renderCategoryCard = (c: NoteCategory) => (
        <div className="group relative overflow-hidden rounded-2xl border border-neutral-200 bg-white p-5 transition-all hover:shadow-xl dark:border-neutral-800 dark:bg-neutral-900">
            <div className="absolute top-4 right-4">
                <TableRowActions id={c.id} label={c.name} edit={{ onClick: () => openEditModal(c) }} deleteRoute="note-categories.destroy" />
            </div>
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="space-y-4 p-4">
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className="flex items-center gap-2.5">
                        ...
                        <h1 className="text-lg font-semibold text-neutral-900 dark:text-neutral-100">Note Categories</h1>
                    </div>
            ...
            <SaveConfirmationModal
                ...
                description="Are you sure you want to save these changes to the note category?"
            ...
            >
                <form onSubmit={handleSubmit} className="space-y-4">
                    <FormInput
                        ...
                        placeholder="e.g. Meeting Minutes, Ideas"
                        error={errors.name}
                    />
