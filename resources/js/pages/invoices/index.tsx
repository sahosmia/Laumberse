...
import { CircleCheck, CircleDollarSign, Plus, Printer, Search, Trash2 } from 'lucide-react';
                ...
                <div className="overflow-hidden rounded-2xl border border-neutral-200 bg-white dark:border-neutral-800 dark:bg-neutral-900">
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="bg-neutral-50 text-xs tracking-wider text-neutral-500 uppercase dark:bg-neutral-800/50">
                                    ...
                                    <th className="px-3 py-3 text-left font-semibold">Invoice UUID</th>
                                    ...
                                    <th className="px-3 py-3 text-center font-semibold">Payment</th>
                            ...
                            <tbody>
                                {invoices.data.map((inv) => (
                                    <tr
                                        key={inv.id}
                                        className={`border-b border-neutral-50 transition-colors hover:bg-neutral-50/50 dark:border-neutral-800 dark:hover:bg-neutral-800/30 ${selectedIds.includes(inv.id) ? 'bg-blue-50/50 dark:bg-blue-900/10' : ''}`}
                                    >
                                        ...
                                        <td className="px-3 py-3 text-right font-medium text-emerald-600">{formatCurrency(Number(inv.paid))}</td>
                    ...
                    <div className="divide-y divide-neutral-100 md:hidden dark:divide-neutral-800">
                        {invoices.data.map((inv) => (
                            <div
                                key={inv.id}
                                className={`space-y-3 bg-white p-4 dark:bg-neutral-900 ${selectedIds.includes(inv.id) ? 'bg-blue-50/50 dark:bg-blue-900/10' : ''}`}
                            >
                                <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-start justify-between">
                                    ...
                                    <div className="text-right">
                                        ...
                                        <p className="text-xs font-medium text-emerald-600">Paid: {formatCurrency(Number(inv.paid))}</p>
                                    </div>
                                </div>
                                <div className="flex items-center justify-between border-t border-neutral-50 pt-2 dark:border-neutral-800/50">
                                    <PaymentStatusToggle invoice={inv} />
                                </div>
                                <div className="flex items-center justify-end border-t border-neutral-50 pt-2 dark:border-neutral-800/50">
                                    <TableRowActions
                                        id={inv.id}
                                        label={`invoice ${inv.invoice_uuid}`}
                                        view={{ href: route('invoices.show', inv.id) }}
