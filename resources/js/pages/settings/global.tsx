...
function BusinessInfoForm({ settings, hidden }: { settings: GlobalSettingsValues; hidden: boolean }) {
    ...
    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        patch(route('settings.global.business.update'));
    };

    return (
        <form onSubmit={submit} className={cn('space-y-4', hidden && 'hidden')}>
...
function BusinessWeekForm({ settings, hidden }: { settings: GlobalSettingsValues; hidden: boolean }) {
    const { data, setData, patch, errors, processing, recentlySuccessful } = useForm({
    ...
    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        patch(route('settings.global.week.update'));
    };
...
function BrandingForm({ settings, hidden }: { settings: GlobalSettingsValues; hidden: boolean }) {
    const { data, setData, patch, errors, processing, recentlySuccessful } = useForm({
    ...
    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        patch(route('settings.global.branding.update'));
    };
...
function CategoriesForm({
    ...
    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        patch(route('settings.global.categories.update'));
    };
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="space-y-4 p-4">
                ...
                <div className="max-w-2xl space-y-4">
                    ...
                    {/* Each tab is its own independent form with its own submit — saving one
                        section (e.g. Branding) never touches or re-validates the others. All four
                        stay mounted (just visually hidden) so switching tabs never discards an
                        unsaved draft in another one. */}
                    <BusinessInfoForm settings={settings} hidden={activeTab !== 'business'} />
                    <BusinessWeekForm settings={settings} hidden={activeTab !== 'week'} />
                    <BrandingForm settings={settings} hidden={activeTab !== 'branding'} />
                    <CategoriesForm settings={settings} expense_categories={expense_categories} hidden={activeTab !== 'categories'} />
                </div>
            </div>
        </AppLayout>
    );
